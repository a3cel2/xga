...
########                                                ########
########                                                ########
########     Pipeline for XGA, performs all             ########
########     computational analyses and generates       ########
########     most figures.                              ########
########                                                ########
########                                                ########
################################################################
...
#  'resistance' - compares growth in each drug to control (used in paper)
...
#
# 'Merge mating types' - combines data from
...
#
# 'NN optimization'
#   hyperparameter search (L1 regularization rate)
#   for neural network construction
#
# 'NN analysis'
#   main neural network construction and evaluation
#
# 'NN extensions'
#   'extended' neural network construction for
#   valinoymcin and fluconazole

to_run <- c('NN analysis')#Radial Fitness Landscape')#,'NN extensions')#'qRT-PCR' )#'Linear model')#'Fitness Density Plot')#'Merge mating types')#'Genotyping Accuracy')#)#)#'Create resistance metrics','Pool Genotype Group Comparison')
...
if ('Create resistance metrics' %in% to_run | 'all' %in% to_run) {
  ...
  ##Make a list for input to calculate_log_resistance
  log_res_input_list <- make_input_list(input_file,drugs)
  ...
  for(drug in drugs){
    ...
    log_res <- calculate_log_resistance(log_res_input_list,drug, metric_returned = growth_metric)
...
if ('Linear model' %in% to_run | 'all' %in% to_run) {
  ...
  #Values have to be above 0 for glm
...
if ('Linear coefficient heatmap' %in% to_run | 'all' %in% to_run) {
  ...
  existing_associations <- read.xlsx2('../../data/output/drug-knockout-associations.xlsx', sheetIndex = 1)
...
if ('Fitness Density Plot' %in% to_run | 'all' %in% to_run) {
  ...
  draw_split_density_distribution(
    ...
    swarm_size = 0.01,
    cex_axis = 1.3,
    genotype_circle_width_constant = 0.095
  )
  dev.off()
...
if ('Analyze Tecan Data' %in% to_run | 'all' %in% to_run) {
  ...
  correspondence_dir <- 'tecan_fluconazole'
  setwd(this.dir)
  setwd(paste(
    c(input_data_directory, correspondence_dir),
    collapse = ''
  ))
...
if('NN analysis' %in% to_run | 'all' %in% to_run){
  ...
  #Get subset of weights for valinomycin figure
  ...
  #Get subset of weights for fluconazole figure
  ...
  correl <- format(correl, digits = 2)
  correl <- paste(c('r = ',correl),collapse='')
  text(0,0.9,correl,cex=3,adj=0,col='white')
  ...
  setwd(this.dir)
  setwd(output_data_directory)
  save('named_weights',file='named_weights_for_nn.RData')
...
if('NN extensions' %in% to_run | 'all' %in% to_run){
  ...
  if(filename %in% list.files()){
    ...
    search_reg_rate_three_layer <- nn_model_search(resfile = both_res_file,
                                                   ...
                                                   epochs = 10000,
                                                   learning_rate = 0.01,
                                                   regularization_rates = 10^seq(-6,-4,by=0.2))
    
    search_reg_rate_two_layer <- nn_model_search(resfile = both_res_file,
                                                   mapfile = mapfile,
                                                   genes = modifier_genes,
                                                   efflux_genes = 'PDR5',
                                                   condition_names = 'fluconazole_both',
    ...
    save(file=filename, list=c('search_reg_rate_three_layer','search_reg_rate_two_layer'))
  }
  ...
  plot(
    ...
    (search_reg_rate_three_layer[, 6]),
    type = 'l',
    col = 'purple',
    xlab = expression(paste("log" ["10"], " (Regularization rate)")),
    ylab = 'Mean Squared Error',
    cex.lab = 1.5,
    lwd = 2
  )
  lines(
    log10(search_reg_rate_two_layer[, 1]),
    (search_reg_rate_two_layer[, 6]),
    type = 'l',
    col = 'red',
    xlab = expression(paste("log" ["10"], " (Regularization rate)")),
    ylab = 'Mean Squared Error',
  ...
  draw_model_diagram(
    ...
    influence_line_width_scale_factor = 4,
    influence_arrowhead_scale_factor = 0.05
  ...
  named_weights_fluc_single_envir <- name_model_weights(get_weights(nn_fluc$model), genes = modifier_genes, efflux_genes = efflux_genes, condition_names = 'fluconazole')
