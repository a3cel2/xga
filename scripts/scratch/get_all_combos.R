# #alpha_res_filtered <- alpha_resistance_file[!is.na(apply(alpha_resistance_file,1,sum)),]
# #A_res_filtered <- A_resistance_file[!is.na(apply(A_resistance_file,1,sum)),]
# 
...
# 
# 
# combos <- t(replicate(100000,replicate(16,sample(c(1,2,3),1))))
# 
...
create_many_comparison_combos <- function(combos,
  ...
  group_profile <- t(sapply(which(testme)[1:ngroups_returned],function(ind){
    ...
    means1 <- apply(A_res_filtered[has_kos_A & has_wts_A, ], 2, mean, na.rm=T)
    means2 <- apply(alpha_res_filtered[has_kos_alpha & has_wts_alpha, ], 2, mean, na.rm=T)
